...
//! DeleteShareGroupOffsets`: answered from the same `kafgres_share_offsets` table ShareFetch
//! reads positions from, so a reset also clears the inflight state it touches.
...
/// Kafka reports a partition with no stored position as start offset -1 and lag -1, not 0.
...
pub fn describe(
    ...
    // I8: the cap is on the product of groups and partitions; `topics: null` expands from stored state.
    ...
    for g in &req.groups {
        ...
        for (name, partitions) in wanted {
            ...
            for partition in partitions {
                ...
                // Unknown topic is NONE with a -1 start offset here; key 90 does not list UNKNOWN_TOPIC_OR_PARTITION.
    ...
    for t in &req.topics {
        ...
        for p in &t.partitions {
            let code = match (denied, topic_id) {
                ...
                // Gated on the partition count; otherwise a reset writes a row no ShareFetch reads.
